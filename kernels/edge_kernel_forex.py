...
class DiffusionKernelForex(Kernel):
    def __init__(self, eigenpairs, kappa_bounds=(1e-5,1e5)): 
        super().__init__()
        self.harm_eigvects, self.grad_eigvects, self.curl_eigvects, self.harm_eigvals, self.grad_eigvals, self.curl_eigvals = eigenpairs
...
class DiffusionKernelForexNonHC(Kernel):
    def __init__(self, eigenpairs, kappa_bounds=(1e-5,1e5)): 
        super().__init__()
        self.harm_eigvects, self.grad_eigvects, self.curl_eigvects, self.harm_eigvals, self.grad_eigvals, self.curl_eigvals = eigenpairs
        # register the raw parameters
...
class MaternKernelForex(Kernel):
    def __init__(self, eigenpairs, kappa_bounds=(1e-5,1e8)): 
        super().__init__()
        self.harm_eigvects, self.grad_eigvects, self.curl_eigvects, self.harm_eigvals, self.grad_eigvals, self.curl_eigvals = eigenpairs
        # register the raw parameters
        self.register_parameter(
    ...
    def _eval_covar_matrix(self):
        """Define the full covariance matrix -- full kernel matrix as a property to avoid repeative computation of the kernel matrix"""
        # k0 = torch.pow(2*self.mu/self.kappa/self.kappa + self.harm_eigvals, -self.mu)
        k0 = self.mu.expand(self.harm_eigvals.shape[0],-1).squeeze() # equivalent to above 
...
class MaternKernelForexNonHC(Kernel):
    def __init__(self, eigenpairs, kappa_bounds=(1e-5,1e8)): 
        super().__init__()
        self.harm_eigvects, self.grad_eigvects, self.curl_eigvects, self.harm_eigvals, self.grad_eigvals, self.curl_eigvals = eigenpairs
        # register the raw parameters
    ...
    def _eval_covar_matrix(self):
        """Define the full covariance matrix -- full kernel matrix as a property to avoid repeative computation of the kernel matrix"""
        k0 = torch.pow(2*self.mu_down/self.kappa_down/self.kappa_down + self.harm_eigvals, -self.mu_down)
        k1 = torch.pow(2*self.mu_down/self.kappa_down/self.kappa_down + self.grad_eigvals, -self.mu_down) 
